...
export const geolocationReducer = createReducer(
  ...
  on(fetchGeolocationSuccess, (state, {geolocation}) => ({...state, fetchedGeolocation: false, geolocation: geolocation})),
  on(fetchGeolocationError, (state) => ({...state, fetchedGeolocation: false}))
)
